...
subfolder = '\pRTransition_analysis1';
...
transitions = {'RestREM_to_ActiveREM'};
...
ActiveREMonset = REMonset + [26*SR, 20*SR];
PhasicREMonset = round(EM_onset_manual/100);

label1 = label;
label2 = label;
for i = 1:length(REMonset)
    label1(PhasicREMonset(i):REMoffset(i)) = 4;
end
clear i;

for i = 1:length(REMonset)
    label2(REMonset(i):ActiveREMonset(i)) = 5;
    label2(ActiveREMonset(i):REMoffset(i)) = 0;
end
clear i;
...
for i = 1:size(ICtrace,1)  % for every IC
   ...
   for tr = 1:length(transitions)   % for every kind of transition
       temp = calculate_transition_tR2pR(char(transitions(tr)),pre,post,labels(:,tr),fluo_signal);
